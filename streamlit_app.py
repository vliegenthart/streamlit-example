...
For internal use only. Please contact Daniel V. on Slack if you have questions or requests.
"""
    ...
    def download_index():
        ...
        if index.isdigit():
            if int(index) > df.shape[0]:
                st.session_state.warning = f"Row index too big, please select number between 1 and {df.shape[0]}"
            else:
                st.session_state.download_row = df.iloc[int(index)]
        else:
            st.session_state.warning = f"Not a valid row index, please select number between 1 and {df.shape[0]}"
